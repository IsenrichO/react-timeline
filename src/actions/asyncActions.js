...
export const crudAsync2 = (
  operation,
  endpoint = RoutePaths.Events,
  dispatch,
  actionCreator,
  curriedArgs,
  ...configOpts
  ...
  //   .then((resp) => {
  //     console.log('\n\n\nRESPONSE:', resp);
  //     console.log('RESPONSE DATA:', resp.config.data);
  ...
  return operation(endpoint, curriedArgs)
    .then((resp) => !!actionCreator
      ? dispatch(actionCreator(resp.data))
      : null,
    )
    .catch((err) => {
      throw new Error(`
        An error was encountered while making request to the <${endpoint}> endpoint:
        > Error Message:\t${err}
      `);
    });
};
