...
By a similar token, the parameter(s) passed to a chained `then()` call preempted by Mongoose's `findOne()` method should reflect the fact that only a single item will be curried to the `then`able:
...
it('should not curry unused arguments', (done) => {
  // The `done` argument goes unused...
});
```

will complain inside the terminal:

```
Error: Timeout of 2000ms exceeded. For async tests and hooks, ensure "done()" is called; if returning a Promise, ensure it resolves.
```

Instead, ensure either your `done` callback is being used (if such is your intention) or simply do not include it:

```javascript
it('should not curry unused arguments', (done) => {
  const newUser = new User({ name: 'Oliver' });
  newUser
    .save()
    .then(() => {
